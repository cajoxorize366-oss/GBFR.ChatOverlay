...
- Keep the microphone synchronously muted and verified by default. DirectInput consumes `U` for the voice test. Each U hold opens the configured Windows microphone in WASAPI shared mode, downmixes and resamples it, and submits exact 960-sample/3,840-byte frames through `PartyAudioManipulationSinkStreamSubmitBuffer`. A monotonic publication deadline permits at most one frame per 40 ms; a late sender establishes a fresh deadline instead of replaying missed intervals in a burst. These buffers become this ChatControl's microphone input and use the already authenticated PartyNetwork; no Relink gameplay endpoint payload is constructed.
- Party error `0x10D8` means that the audio stream currently lacks queue space. It is handled as recoverable backpressure: the current 40 ms frame is dropped, counted and retried on the next paced frame. Logging is limited to power-of-two drop counts. Other consecutive submit errors still trigger the three-strike fail-closed path.
- U release synchronously closes the managed submission gate before Party mute is queued. WASAPI stop/disposal then runs off the input and Party state-change threads. Old callbacks carry a per-hold epoch and cannot submit into a later hold or session.
...
- Preview.9 retains the completely separate hold-`I` local microphone monitor. It opens the configured Windows capture and render endpoints in WASAPI shared mode, copies microphone samples only to the local playback device, and measures a local peak for UI/log evidence. Release closes a one-way silence gate without taking the audio-buffer lock; potentially blocking NAudio endpoint cleanup runs on a dedicated background thread and cannot delay a subsequent `I` hold. It never calls Party, changes chat permissions or sends audio over the network. `U` has priority, and both paths are fail-closed on release, focus loss, input timeout, suspend or endpoint failure.
- The Party troubleshooting sampler remains read-only, while the capture bridge separately records every accepted Party frame, submitted duration, peak, failure and state-batch skip. The old automatic-input `NoAudioInput` indicator is diagnostic context rather than a local failure once the official manipulation sink has accepted PCM. A complete online pass requires both local sink evidence and one specific remote ChatControl satisfying permission, incoming-unmuted, positive render volume and `Talking`; evidence from different peers cannot be combined. See `VOICE_TROUBLESHOOTING_MATRIX.md`.
