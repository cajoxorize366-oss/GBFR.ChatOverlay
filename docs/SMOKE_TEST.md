...
While the `F10` menu is open, drag the cyan top edge of the chat preview and drag the cyan triangle in its bottom-right corner. Close the menu, reopen it, and confirm position/size persist. Repeat once at another resolution or window size: the chat box should retain the same relative placement inside the usable viewport and remain fully on-screen. Hold a keyboard key or mouse button while closing the menu; the held input must not leak into the game, and normal input must resume after physical release.
