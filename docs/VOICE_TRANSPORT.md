...
- The external voice test is implemented and enabled by default in `0.3.0-preview.8`. Every participant must install the same package; a vanilla peer has no remote ChatControl and is not granted voice permissions.
...
- Keep the microphone synchronously muted and verified by default. DirectInput consumes `U` for the voice test. Each U hold opens the configured Windows microphone in WASAPI shared mode, downmixes and resamples it, and submits exact 960-sample/3,840-byte frames every 40 ms through `PartyAudioManipulationSinkStreamSubmitBuffer`. These buffers become this ChatControl's microphone input and use the already authenticated PartyNetwork; no Relink gameplay endpoint payload is constructed.
