...
Opening chat has two deliberately separate phases. The open request immediately enables IME lifecycle routing so `WM_IME_SETCONTEXT` and composition notifications are not lost between the hotkey message and the next Present frame. Committed `WM_CHAR` or `WM_IME_CHAR` text is accepted only after the composer actually exists. Text generated by the activation key is discarded until that key is physically released, and keyboard focus is assigned on the following frame so the opening keystroke cannot enter the draft.

## Focus and safety

`WM_SETFOCUS`, `WM_KILLFOCUS`, `WM_ACTIVATE`, and `WM_ACTIVATEAPP` are observed for local cleanup and then forwarded through the Overlay Broker to both Dear ImGui and Relink. Window focus loss clears pending binding capture, activation-key suppression, incomplete DBCS/composition state, and forces push-to-talk muted. When an open composer regains focus, its input field is focused again on the next frame. Any broker ABI mismatch or native failure changes the broker to fail-open for game input and fail-closed for mod hotkeys.
