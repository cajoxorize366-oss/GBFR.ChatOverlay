...
- `0.4.0` completes the position-only party HUD microphone overlay and the live-memory-verified full HP-row battle anchors. At 2560x1440 it renders the indicator at roughly 48 px and keeps its center approximately 32 px beyond the HP-row endpoint, including the final 8 px release adjustment; other resolutions continue to follow the game's native HUD transform. Its default rule is a strict native party-HUD whitelist: an anchor is emitted only when the live `ControllerPlParameterTown`/`ControllerPlParameter01` visibility state at `+0x188` equals the stable-visible value `2` and its selected HP-row node is active. Opening state `1` no longer draws early over load screens, and closing state `3` hides immediately; menus, results and every other screen without the complete party HP HUD submit no foreground indicator draw commands. Full Chain is one verified game exception because its illustration leaves those HP rows rendered underneath. The release therefore retains one narrow blacklist patch: while the actual `ControllerChainburst` visibility state at `+0x188` is nonzero, every microphone anchor is suppressed through its opening, visible and closing phases, then the normal HP-HUD whitelist resumes. Its default debug override shows every live CPU or player HUD row so lobby and battle placement can be checked without peers. Placement follows the game's active `ControllerPlParameterTown`/`ControllerPlParameter01` child-node transforms and current viewport rather than screenshot coordinates or a reference-resolution scale; lobby/battle selection is automatic. Idle indicators retain 70% Alpha but use a muted palette; Speaking uses the bright 100% palette. With the override disabled, all per-slot icons remain hidden until explicit capability negotiation is implemented; the release never guesses a remote slot from ChatControl timing or display name.
