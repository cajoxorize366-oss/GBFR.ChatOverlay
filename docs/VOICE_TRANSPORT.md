...
- `0.4.0-preview.1` adds a position-only party HUD microphone overlay. Its default debug override shows every live CPU or player HUD row so lobby and battle placement can be checked without peers. Placement follows the game's active `ControllerPlParameterTown`/`ControllerPlParameter01` child-node transforms and current viewport rather than screenshot coordinates or a reference-resolution scale; lobby/battle selection is automatic. With the override disabled, all per-slot icons remain hidden until explicit capability and identity mapping are implemented; the preview never guesses a remote slot from ChatControl timing or display name.
