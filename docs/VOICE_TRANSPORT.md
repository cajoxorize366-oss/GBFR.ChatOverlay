...
- The external voice test is implemented and enabled by default. Every intended participant must install the same package. Remote ChatControl observation is not by itself a secure Mod capability proof. The player-mute path can now correlate a ChatControl to a Relink slot by exact Party EntityId, but that identity match does not prove which Mod version or feature set the peer is running.
- Treat explicit Mod capability negotiation as required follow-up work. Do not label CPU, vanilla or merely same-network controls as verified Mod peers.
...
- Reloaded-II and the F10 menu expose independent dynamic lists for active Windows capture and render endpoints. Both lists default to an explicit `Default (Windows system default)` entry, which maps to the Windows default communications endpoint. Manual choices save the stable `IMMDevice` endpoint ID while the UI displays the friendly device name. Legacy blank values migrate to `Default`; a saved endpoint that is no longer active falls back to that default with an explicit startup log. The microphone choice drives Party's U input selection and the F10 self-test's local WASAPI capture; the playback choice drives Party's output route and local self-test route. Party `Manual` selection is accepted only when the completion event confirms the exact endpoint ID before the ChatControl can connect.
- The chat overlay has a persistent voice status row for session wait, ChatControl initialization, remote wait, ready, speaking, disconnect and fail-closed states. Compact Mode hides the whole chat frame while closed, but both its live `Y` input shape and its `F10` edit preview retain this voice row directly above the input. `Speaking` is derived from the lock-protected canary state only after `SetAudioInputMuted(false)` succeeds and `GetAudioInputMuted` verifies `false`; a raw `U` key-down is not sufficient.
- `0.5.0-preview.22` implements the production party-HUD microphone candidate loop on top of the live-memory-verified `0.4.0` anchors. One coherent Relink four-member EntityId snapshot is combined with the current permissioned Party ChatControls and read-only `Talking` indicators, then published as a single immutable state for the overlay tick/render boundary. Established local and exact remote members whose permissioned ChatControl supplied the matching EntityId use the muted 70% indicator; Party `Talking` uses the bright 100% state. A remote icon is never inferred from ChatControl join order, display name or a CPU row, and the indicator does not claim an authenticated Mod capability proof. The row mapper handles the two currently observed HUD shapes: three fixed remote rows map to remote ordinals 1-3, while occupied-only layouts zip sorted occupied ordinals to sorted visible remote rows; every other cardinality, mixed layout, invalid geometry, missing identity or getter failure hides the formal indicators fail-closed. The native whitelist remains unchanged: only stable-visible `ControllerPlParameterTown`/`ControllerPlParameter01` anchors are emitted, menus/loading/results remain empty, and a nonzero `ControllerChainburst` state suppresses every icon for the full Full Chain sequence. Snapshot and native row/type transitions are logged for two-client validation. The diagnostics-only show-all override still draws every valid CPU/player row for position checks and does not prove the formal channel loop. Live two-client town and battle identity placement remains `UNVERIFIED` until the smoke test below is completed.
