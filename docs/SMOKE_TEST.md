...
- Restart the game with `0.5.0-preview.20`. On both clients trigger `vo_CMM_chance`, `vo_CMM_win_3` and `vo_CMM_thanks`, then send distinguishable ordinary text from each side. Every cue and text line must retain its actual sender even when the local Party slot is nonzero; neither identity nor any room name may contain `vo_CMM_*`. A process that already loaded an older DLL must be restarted because its in-memory identity and lobby binding may already be stale.
