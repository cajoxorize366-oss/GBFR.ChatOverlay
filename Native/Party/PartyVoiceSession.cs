...
internal enum PartyVoiceSessionPhase
{
...
/// The control remains muted until a remote Mod ChatControl is present and push-to-talk is held.
/// Only microphone send/receive permissions are granted to remote ChatControls on the same network.
/// Native actions never overlap the game's state-change batch.
/// </summary>
internal sealed class PartyVoiceSession : IDisposable
{
    ...
    private PartyVoiceSessionPhase _phase =
        PartyVoiceSessionPhase.WaitingForAuthenticatedSession;
    ...
    internal PartyVoiceSessionPhase Phase
    {
        get
        {
            lock (_stateSync)
    ...
    public void CaptureManager(nint manager, string source)
    {
        ...
        lock (_stateSync)
        {
            if (_manager == nint.Zero)
            {
                ...
                _phase = PartyVoiceSessionPhase.WaitingForAuthenticatedSession;
                _generation++;
                return;
            }
    ...
    public void OnBatchFinished(nint manager)
    {
        lock (_stateSync)
        {
                ...
                _localChatControl != nint.Zero)
            {
                ...
                _phase = PartyVoiceSessionPhase.Completed;
                _sessionFaulted = true;
                _generation++;
                EnqueueLogLocked(
                    "Party voice session cleanup complete: local ChatControlDestroyed was returned to PartyFinishProcessingStateChanges.");
            }
        }
    ...
    public void SetPushToTalkPressed(bool pressed)
    {
        lock (_stateSync)
        {
            if (_disposed != 0)
                return;
            var normalizedPressed = pressed && IsRemoteVoiceReadyLocked();
            var changed = normalizedPressed != _pushToTalkPressed;
            _pushToTalkPressed = normalizedPressed;
            if (pressed && changed && normalizedPressed)
            {
                EnqueueLogLocked(
                    "Party voice push-to-talk press accepted; Party input unmute was queued behind the current state batch.");
            }
            if (changed && _localChatControl != nint.Zero)
        ...
        lock (_apiCallSync)
        {
            ...
            lock (_stateSync)
            {
                ...
                if (_stateBatchActive)
                {
                    ...
                    _phase = PartyVoiceSessionPhase.Disabled;
                    _generation++;
                    EnqueueVoiceDiagnosticSummaryLocked("network leave during Party state batch");
                    EnqueueLogLocked(
                        "Party voice session observed PartyNetworkLeaveNetwork while a Party state-change batch was active. " +
                        "The voice session issued no overlapping Party calls; Relink's original network leave owns " +
                        "mute/disconnect/destruction and voice session calls remain disabled until manager cleanup.");
                    return;
                }
                ...
                {
                    EnqueueLogLocked(
                        $"Party voice session observed Relink PartyNetworkLeaveNetwork for tracked network={Hex(network)} " +
                        "before a local ChatControl existed; no voice session teardown call was needed.");
                    return;
                }
                ...
                // _apiCallSync ensures no voice session native call overlaps this pre-leave operation.
                _generation++;
                _destroyQueued = true;
                _phase = PartyVoiceSessionPhase.Destroying;
            ...
            catch (Exception exception)
            {
                ...
                lock (_stateSync)
                {
                    if (_localChatControl == localChatControl && _localDevice == localDevice)
                    {
                        ...
                        _generation++;
                        EnqueueLogLocked(
                            $"Party voice session pre-leave native teardown threw {exception.GetType().Name}: " +
            ...
            lock (_stateSync)
            {
                ...
                if (muteResult != Success)
                {
                    _sessionFaulted = true;
                    EnqueueLogLocked(
                        $"Party voice session pre-leave PartyChatControlSetAudioInputMuted(true) returned " +
                ...
                if (destroyResult != Success)
                {
                    ...
                    _generation++;
                    EnqueueLogLocked(
                        $"Party voice session pre-leave PartyDeviceDestroyChatControl returned 0x{destroyResult:X8}; " +
                ...
                EnqueueLogLocked(
                    $"Party voice session pre-leave DestroyChatControl queued before Relink PartyNetworkLeaveNetwork: " +
            ...
            lock (_stateSync)
            {
                ...
                if (chatControl != nint.Zero && chatControl == _localChatControl)
                {
                    ...
                    else
                    {
                        ...
                        EnqueueLogLocked(
                            $"Party voice manager-cleanup emergency mute {failure}; PartyCleanup owns final teardown.");
                    }
                }
                ...
                {
                    EnqueueLogLocked(
                        "Party voice session manager cleanup reached before local ChatControl teardown completed: " +
                ...
                _stateBatchManager = nint.Zero;
                EnqueueLogLocked(
                    "Party voice session manager cleanup began; voice session native calls are now blocked and Party owns final teardown.");
            }
        }
    }
    ...
    public void CompleteManagerCleanup(nint manager, bool succeeded)
    {
        lock (_stateSync)
        {
            ...
            if (!succeeded)
            {
                FailClosedLocked("PartyCleanup failed after voice session shutdown began");
                return;
            }

            ResetForNextManagerLocked();
            EnqueueLogLocked("Party voice session manager cleanup complete; cached user/network/device handles were cleared.");
        }
    }
    ...
    public void SuspendBestEffort()
    {
        lock (_apiCallSync)
        {
            ...
            {
                lock (_stateSync)
                    EnqueueLogLocked($"Party voice suspend emergency mute threw {exception.GetType().Name}: {exception.Message}.");
            }
            ...
            if (device != nint.Zero && !destroyAlreadyQueued)
            {
                ...
                {
                    lock (_stateSync)
                        EnqueueLogLocked($"Party voice session suspend ChatControl destruction threw {exception.GetType().Name}: {exception.Message}.");
                }
            }
    ...
    public void ResumeFailClosed()
    {
        lock (_stateSync)
        {
            ...
            if (_manager != nint.Zero)
            {
                ...
                _sessionFaulted = true;
                EnqueueLogLocked(
                    "Party voice session remains disabled after resume until Party creates a new manager; suspended events were not observed.");
            }
        }
    }
    ...
    private void ObserveAuthenticationLocked(PartyStateChangeSnapshot state)
    {
        ...
        {
            EnqueueLogLocked(
                $"Party voice session prerequisite AuthenticateLocalUserCompleted failed: result={state.Result}, " +
            ...
            (_localUser != nint.Zero && _localUser != state.LocalUser))
        {
            ...
            {
                FailClosedLocked(
                    $"authenticated session changed while voice session state was active: " +
        ...
        _authenticated = true;
        EnqueueLogLocked(
            $"Party voice session captured authenticated existing session: network={Hex(_network)}, localUser={Hex(_localUser)}.");
    }
    ...
    private void ObserveEndpointLocked(PartyStateChangeSnapshot state)
    {
        ...
        _endpointReady = true;
        EnqueueLogLocked(
            $"Party voice session confirmed Relink's existing gameplay endpoint before voice session creation: endpoint={Hex(state.Endpoint)}.");
    }
    ...
    private void ObserveCreateCompletedLocked(PartyStateChangeSnapshot state)
    {
        ...
        EnqueueLogLocked(
            $"Party voice session CreateChatControlCompleted: result={state.Result}, error=0x{state.ErrorDetail:X8}, " +
        ...
        {
            RequestTeardownLocked("CreateChatControlCompleted did not match the owned voice session operation");
            return;
        }
    ...
    private void ObserveChatControlCreatedLocked(PartyStateChangeSnapshot state)
    {
        ...
        if (state.ChatControl == _localChatControl)
        {
            ...
            EnqueueLogLocked(
                $"Party voice session ChatControlCreated (local owned control): chatControl={Hex(state.ChatControl)}.");
        }
        else
        {
            EnqueueLogLocked(
                $"Party voice session ChatControlCreated (remote/other): chatControl={Hex(state.ChatControl)}.");
        }
    }
    ...
    private void ObserveAudioCompletedLocked(PartyStateChangeSnapshot state, bool input)
    {
        ...
        EnqueueLogLocked(
            $"Party voice session {label}: result={state.Result}, error=0x{state.ErrorDetail:X8}, " +
    ...
    private void ObserveAudioStateChangedLocked(PartyStateChangeSnapshot state, bool input)
    {
        ...
        if (input)
        {
            ...
            EnqueueLogLocked(
                $"Party voice audio input state: {FormatEnum(inputState)}; " +
        ...
        else
        {
            ...
            EnqueueLogLocked(
                $"Party voice audio output state: {FormatEnum(outputState)}; " +
                $"errorDetail=0x{state.ErrorDetail:X8}. Expected healthy state: Initialized (1).");
        }
        ...
        if (!AreAudioStatesInitializedLocked())
        {
            if (_pushToTalkPressed || _inputUnmuted || _microphoneMayBeOpen)
            {
                _pushToTalkPressed = false;
                EnqueueLogLocked(
                    "Party voice audio device state is not Initialized; the push-to-talk hold was revoked " +
    ...
    private void ObserveConnectCompletedLocked(PartyStateChangeSnapshot state)
    {
        ...
        EnqueueLogLocked(
            $"Party voice session ConnectChatControlCompleted: result={state.Result}, error=0x{state.ErrorDetail:X8}, " +
            $"network={Hex(state.Network)}, chatControl={Hex(state.ChatControl)}.");
        if (state.Result != SucceededStateChange ||
            state.Network != _network ||
            state.ChatControl != _localChatControl)
        {
            RequestTeardownLocked("ConnectChatControlCompleted did not match the owned voice session operation");
            return;
        }
    ...
    private void ObserveJoinedLocked(PartyStateChangeSnapshot state)
    {
        ...
        if (state.ChatControl == _localChatControl && state.Network == _network)
        {
            ...
            EnqueueLogLocked(
                $"Party voice session ChatControlJoinedNetwork (local owned control): network={Hex(state.Network)}, " +
                $"chatControl={Hex(state.ChatControl)}.");
        }
        else
        {
            ...
            EnqueueLogLocked(
                $"Party voice session ChatControlJoinedNetwork (remote/other): network={Hex(state.Network)}, " +
                $"chatControl={Hex(state.ChatControl)}.");
        }
    }
    ...
    private void ObserveDisconnectCompletedLocked(PartyStateChangeSnapshot state)
    {
        ...
        EnqueueLogLocked(
            $"Party voice session DisconnectChatControlCompleted: result={state.Result}, error=0x{state.ErrorDetail:X8}.");
        if (state.Result != SucceededStateChange ||
            state.ChatControl != _localChatControl ||
            state.Network != _network)
        {
    ...
    private void ObserveLeftLocked(PartyStateChangeSnapshot state)
    {
        if (state.ChatControl == _localChatControl)
        {
            ...
            _teardownRequested = true;
            EnqueueLogLocked(
                $"Party voice session ChatControlLeftNetwork (local owned control): reason={state.Reason}, " +
        ...
        else if (state.ChatControl != nint.Zero)
        {
            if (state.Network == _network)
            {
                ...
                if (_permissionedRemoteChatControls.Count == 0 &&
                    _phase == PartyVoiceSessionPhase.VoiceReady)
                {
                    _phase = PartyVoiceSessionPhase.JoinedMuted;
                    _pushToTalkPressed = false;
                }
            ...
            EnqueueLogLocked(
                $"Party voice session ChatControlLeftNetwork (remote/other): reason={state.Reason}, " +
                $"network={Hex(state.Network)}, chatControl={Hex(state.ChatControl)}.");
        }
    }
    ...
    private void ObserveDestroyCompletedLocked(PartyStateChangeSnapshot state)
    {
        ...
        EnqueueLogLocked(
            $"Party voice session DestroyChatControlCompleted: result={state.Result}, error=0x{state.ErrorDetail:X8}.");
        ...
        {
            FailClosedLocked("DestroyChatControlCompleted did not confirm the owned voice session operation");
        }
    }
    ...
    private void ObserveDestroyedLocked(PartyStateChangeSnapshot state)
    {
        if (state.ChatControl == _localChatControl && _localChatControl != nint.Zero)
        {
            ...
            EnqueueLogLocked(
                $"Party voice session ChatControlDestroyed (local owned control): reason={state.Reason}, " +
                $"error=0x{state.ErrorDetail:X8}.");
        }
        else if (state.ChatControl != nint.Zero)
        {
            ...
            _talkingRemoteEntityIds.Clear();
            EnqueueLogLocked(
                $"Party voice session ChatControlDestroyed (remote/other): reason={state.Reason}, " +
                $"chatControl={Hex(state.ChatControl)}.");
        }
    }
    ...
    private void ObserveLocalUserDestroyedLocked(PartyStateChangeSnapshot state)
    {
        ...
        if (_localChatControl != nint.Zero && !_destroyedObserved)
        {
            ...
            _peerVoiceDiagnosticEvidence.Clear();
            EnqueueLogLocked(
                "Party voice session local user was destroyed before local ChatControl cleanup completed; native calls stopped.");
        }
    }

    private void PromoteJoinedLocked()
    {
        if (_phase == PartyVoiceSessionPhase.Connecting &&
            _connectCompleted &&
            _joinedObserved)
        {
            _phase = PartyVoiceSessionPhase.JoinedMuted;
            EnqueueLogLocked(
                "Party voice session joined the existing PartyNetwork with input muted; microphone permissions " +
                "wait for a remote Mod ChatControl on this same network.");
        }
    }
    ...
    private void TryScheduleWork()
    {
        ...
        lock (_stateSync)
        {
            ...
            if (work.Kind == VoiceWorkKind.None)
                return;

            _workScheduled = 1;
            switch (work.Kind)
            {
                case VoiceWorkKind.Create:
                    _phase = PartyVoiceSessionPhase.Creating;
                    break;
                case VoiceWorkKind.Connect:
                    _phase = PartyVoiceSessionPhase.Connecting;
                    break;
                case VoiceWorkKind.Disconnect:
                    _disconnectQueued = true;
                    _phase = PartyVoiceSessionPhase.Disconnecting;
                    break;
                case VoiceWorkKind.Destroy:
                    _destroyQueued = true;
                    _phase = PartyVoiceSessionPhase.Destroying;
                    break;
                case VoiceWorkKind.GrantVoicePermissions:
                case VoiceWorkKind.DiscoverNetworkChatControls:
                case VoiceWorkKind.ApplyPushToTalk:
                case VoiceWorkKind.CaptureVoiceDiagnostics:
                    break;
            }
        }
        ...
        catch (Exception exception)
        {
            ...
            {
                _workScheduled = 0;
                FailClosedLocked($"could not schedule deferred voice session work: {exception.Message}");
            }
        }
    }

    private VoiceWorkItem GetNextWorkLocked()
    {
        if (_destroyedObserved)
            return default;
        ...
        if (_teardownRequested && _localChatControl != nint.Zero)
        {
            ...
            {
                return CaptureWorkLocked(VoiceWorkKind.Disconnect);
            }

            if ((!_joinedObserved || _networkLeaving || _leftObserved) && !_destroyQueued && _localDevice != nint.Zero)
                return CaptureWorkLocked(VoiceWorkKind.Destroy);
        }

        if (_sessionFaulted)
            return default;

        if (_phase == PartyVoiceSessionPhase.WaitingForAuthenticatedSession &&
            _authenticated &&
            _endpointReady &&
            !_networkLeaving &&
            _manager != nint.Zero &&
            ...
            _localUser != nint.Zero)
        {
            return CaptureWorkLocked(VoiceWorkKind.Create);
        }

        if (_phase == PartyVoiceSessionPhase.ConfiguringMutedAudio &&
            _createCompleted &&
            _createdObserved &&
            _inputCompleted &&
            _outputCompleted &&
            ...
            !_networkLeaving)
        {
            return CaptureWorkLocked(VoiceWorkKind.Connect);
        }

        if ((_phase == PartyVoiceSessionPhase.JoinedMuted ||
             _phase == PartyVoiceSessionPhase.VoiceReady) &&
            _joinedObserved &&
            !_networkLeaving &&
            _network != nint.Zero &&
            _localChatControl != nint.Zero)
        {
            if (!_networkChatControlsDiscovered)
                return CaptureWorkLocked(VoiceWorkKind.DiscoverNetworkChatControls);
            ...
            foreach (var remoteChatControl in _remoteChatControls)
            {
                if (!_permissionedRemoteChatControls.Contains(remoteChatControl))
                {
                    return CaptureWorkLocked(
                        VoiceWorkKind.GrantVoicePermissions,
            ...
            if (_voiceDiagnosticRequested && _remoteChatControls.Count != 0)
            {
                ...
                return CaptureWorkLocked(VoiceWorkKind.CaptureVoiceDiagnostics);
            }
        }
    ...
    private VoiceWorkItem CaptureWorkLocked(
        VoiceWorkKind kind,
    ...
    private void ExecuteWork(VoiceWorkItem work)
    {
        try
        {
            lock (_apiCallSync)
    ...
    private void ExecuteCreate(VoiceWorkItem work)
    {
        ...
        if (staleCreate)
        {
            ...
            {
                EnqueueLogLocked(
                    "Party voice session create completed after its session became stale; the orphaned " +
        ...
        lock (_stateSync)
        {
            if (!IsWorkCurrentLocked(work))
                return;
            _phase = PartyVoiceSessionPhase.ConfiguringMutedAudio;
            EnqueueLogLocked(
                $"Party voice session creation queued on existing manager/network/device: manager={Hex(work.Manager)}, " +
                ...
                "Party's native selected microphone path is active; microphone permissions remain None " +
                "until a remote Mod ChatControl joins this network.");
        }
    }

    private void ExecuteConnect(VoiceWorkItem work)
    {
        ...
        lock (_stateSync)
        {
            ...
            {
                EnqueueLogLocked(
                    $"Party voice session ConnectChatControl queued for existing network={Hex(work.Network)}, " +
                    $"chatControl={Hex(work.ChatControl)}; awaiting completion and joined events.");
            }
        }
    }

    private void ExecuteDiscoverNetworkChatControls(VoiceWorkItem work)
    {
        uint result;
        nint[] chatControls;
        ...
        lock (_stateSync)
        {
            ...
            {
                EnqueueLogLocked(
                    "Party voice PartyNetworkGetChatControls reconciliation was unavailable (" +
            ...
            {
                EnqueueLogLocked(
                    $"Party voice PartyNetworkGetChatControls reconciliation returned 0x{result:X8}; " +
            ...
            EnqueueLogLocked(
                $"Party voice PartyNetworkGetChatControls reconciliation: total={chatControls.Length}, " +
    ...
    private void ExecuteGrantVoicePermissions(VoiceWorkItem work)
    {
        ...
        lock (_stateSync)
        {
            ...
            EnqueueLogLocked(
                $"Party voice microphone permissions granted for remote ChatControl={Hex(work.TargetChatControl)} " +
                $"on network={Hex(work.Network)}: SendMicrophoneAudio|ReceiveMicrophoneAudio (0x0005). " +
                "Input remains muted until push-to-talk is held.");
        }
    }

    private void ExecutePushToTalk(VoiceWorkItem work)
    {
        if (work.UnmuteInput)
        {
        ...
        lock (_stateSync)
        {
            ...
            else
            {
                ...
                {
                    LogNativeInputHoldSummaryLocked("completed push-to-talk hold");
                    _pttCycleActive = false;
                }
                EnqueueLogLocked(work.UnmuteInput
                    ? "Party voice microphone UNMUTED while push-to-talk is held; Party is capturing the " +
                      "configured Windows microphone directly."
                    : "Party voice microphone muted.");
            }
        }
    ...
    private void LogNativeInputHoldSummaryLocked(string reason)
    {
        EnqueueLogLocked(
            $"Party voice local microphone result ({reason}): " +
            (_pttCycleLocalTalkingObserved
                ...
                : "NOT OBSERVED - Party never reported Talking; speak for at least three seconds " +
                  "during the next push-to-talk hold and check the diagnostic snapshots."));
    }

    private void ExecuteVoiceDiagnosticsNonFatal(VoiceWorkItem work)
    {
        try
        {
        ...
        catch (Exception exception)
        {
            ...
            lock (_stateSync)
            {
                if (IsWorkCurrentLocked(work))
                {
                    ...
                    EnqueueLogLocked(
                        $"Party voice diagnostics were inconclusive because the read-only sampler " +
    ...
    private void ExecuteVoiceDiagnostics(VoiceWorkItem work)
    {
        ...
        lock (_stateSync)
        {
                    ...
                    StringComparison.Ordinal))
            {
                ...
                EnqueueLogLocked($"Party voice diagnostics LOCAL: {localFingerprint}.");
            }
        }
        ...
        foreach (var remoteChatControl in remoteChatControls)
        {
            ...
            lock (_stateSync)
            {
                    ...
                    !string.Equals(previousFingerprint, peerFingerprint, StringComparison.Ordinal))
                {
                    ...
                    EnqueueLogLocked(
                        $"Party voice diagnostics PEER {Hex(remoteChatControl)}: {peerFingerprint}.");
                }
            }
        }
    ...
    private void ExecuteDisconnect(VoiceWorkItem work)
    {
        ...
        if (muteResult != Success)
        {
            ...
            lock (_stateSync)
            {
                ...
                if (destroyResult == Success)
                {
                    ...
                    _phase = PartyVoiceSessionPhase.Destroying;
                    EnqueueLogLocked(
                        $"Party voice disconnect mute returned 0x{muteResult:X8}; " +
                ...
                else
                {
                    ...
                    _generation++;
                    EnqueueLogLocked(
                        $"Party voice disconnect mute returned 0x{muteResult:X8} and emergency " +
                        $"PartyDeviceDestroyChatControl returned 0x{destroyResult:X8}; " +
                        "voice session calls stopped and Party owns final cleanup.");
                }
            }
            return;
        }
        ...
        if (result != Success)
        {
            lock (_stateSync)
            {
                if (IsWorkCurrentLocked(work))
                {
                    ...
                    _teardownRequested = true;
                    EnqueueLogLocked(
                        $"Party voice session PartyNetworkDisconnectChatControl returned 0x{result:X8}; " +
        ...
        lock (_stateSync)
        {
            if (IsWorkCurrentLocked(work))
            {
                _inputUnmuted = false;
                _microphoneMayBeOpen = false;
                EnqueueLogLocked("Party voice session DisconnectChatControl queued; awaiting left-network event.");
            }
        }
    }

    private void ExecuteDestroy(VoiceWorkItem work)
    {
        var muteResult = _api.SetAudioInputMuted(work.ChatControl, muted: true);
        ...
        lock (_stateSync)
        {
            if (IsWorkCurrentLocked(work))
            {
                ...
                EnqueueLogLocked(
                    muteResult == Success
                        ? "Party voice session DestroyChatControl queued; awaiting completed and destroyed events."
                        : $"Party voice session DestroyChatControl queued after the final native mute returned " +
    ...
    {
        if (_phase != PartyVoiceSessionPhase.VoiceReady ||
            !_joinedObserved ||
            _networkLeaving ||
            _localChatControl == nint.Zero ||
    ...
    {
        if (_voiceDiagnosticSummaryLogged)
            return;
        ...
        EnqueueLogLocked(
            $"Party voice diagnostic SUMMARY ({reason}): verdict={verdict}; " +
    ...
    private void FailNativeAction(string operation, uint error, bool hasOwnedChatControl)
    {
        lock (_stateSync)
        {
            EnqueueLogLocked($"Party voice session {operation} returned 0x{error:X8}; voice session is failing closed.");
            _sessionFaulted = true;
            _remoteChatControls.Clear();
            _talkingRemoteEntityIds.Clear();
            ClearEstablishedRemoteEntityIdsLocked();
    ...
    private void RequestTeardownLocked(string reason)
    {
        _pushToTalkPressed = false;
        EnqueueLogLocked(
            $"Party voice session validation failed: {reason}; cleanup requested with microphone mute enforced.");
        _sessionFaulted = true;
        _remoteChatControls.Clear();
        _talkingRemoteEntityIds.Clear();
        ClearEstablishedRemoteEntityIdsLocked();
    ...
    private void RequestEmergencyVoiceTeardownLocked(string reason)
    {
        _pushToTalkPressed = false;
        _inputUnmuted = false;
        EnqueueVoiceDiagnosticSummaryLocked("Party voice fail-closed teardown");
        _remoteChatControls.Clear();
        _talkingRemoteEntityIds.Clear();
        ClearEstablishedRemoteEntityIdsLocked();
        _permissionedRemoteChatControls.Clear();
        ...
        _networkLeaving = true;
        EnqueueLogLocked(
            $"Party voice path failed closed: {reason}; microphone was re-muted best-effort and " +
    ...
    private void FailClosedLocked(string reason)
    {
            ...
            !_suspended)
        {
            ...
            _networkLeaving = true;
            EnqueueLogLocked(
                $"Party voice path failed closed while the microphone was open: {reason}; " +
        ...
        else
        {
            _nativeCallsAllowed = false;
            _teardownRequested = false;
            EnqueueLogLocked($"Party voice session disabled (fail-closed): {reason}.");
        }
    }

    private bool IsWorkCurrent(VoiceWorkItem work)
    {
        lock (_stateSync)
            return IsWorkCurrentLocked(work);
    }

    private bool IsWorkCurrentLocked(VoiceWorkItem work)
    {
        if (_disposed != 0 ||
            _suspended ||
            !_nativeCallsAllowed ||
        ...
        if (work.Kind == VoiceWorkKind.GrantVoicePermissions &&
            !_remoteChatControls.Contains(work.TargetChatControl))
        {
            return false;
        }

        return work.Kind != VoiceWorkKind.ApplyPushToTalk ||
    ...
    {
        if (_sessionFaulted || _phase == PartyVoiceSessionPhase.Disabled)
        ...
        {
            PartyVoiceSessionPhase.WaitingForAuthenticatedSession or
            PartyVoiceSessionPhase.Completed =>
                new PartyVoiceUiStatus(PartyVoiceUiState.WaitingForSession),
            PartyVoiceSessionPhase.Creating or
            PartyVoiceSessionPhase.ConfiguringMutedAudio or
            PartyVoiceSessionPhase.Connecting =>
                new PartyVoiceUiStatus(PartyVoiceUiState.Connecting),
            PartyVoiceSessionPhase.JoinedMuted =>
                new PartyVoiceUiStatus(PartyVoiceUiState.WaitingForPeer),
            PartyVoiceSessionPhase.Disconnecting or
            PartyVoiceSessionPhase.Destroying =>
    ...
    private static GCHandle AllocateToken(string operation) =>
        GCHandle.Alloc(new VoiceAsyncToken(operation), GCHandleType.Normal);
    ...
    private sealed record VoiceAsyncToken(string Operation);
    ...
    private readonly record struct VoiceWorkItem(
        VoiceWorkKind Kind,
        long Generation,
        nint Manager,
        nint Network,
