...
The overlay history follows the newest line on its first populated frame, whenever the composer opens, and whenever a newly published message is local. New Party or system messages follow only when the user was already at the bottom; an intentional scrolled-up reading position is preserved.
